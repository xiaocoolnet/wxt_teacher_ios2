...
static CGFloat const leftMargin = 20.0; //left indentation
static CGFloat const rightMargin = 10.0; //left indentation
...
- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        _callButton = [[UIButton alloc] initWithFrame:CGRectMake(self.bounds.size.width - 35, 8, 35, 35)];
        
        [_callButton setImage:[UIImage imageNamed:@"ic_hujiao"] forState:UIControlStateNormal];
        [self addSubview:_callButton];
        
        
        _messageButton = [[UIButton alloc] initWithFrame:CGRectMake(self.bounds.size.width - _callButton.frame.size.width - rightMargin - 35, 8, 35, 35)];
    
        [_messageButton setImage:[UIImage imageNamed:@"ic_xiaoxi"] forState:UIControlStateNormal];
...
- (void)setNode:(YKNodeModel *)node{
    ...
    if (node.isExpand || node.isLeaf) {
        ...
        _leftImage.image = [UIImage imageNamed:@"right"];
    }
...
@property (nonatomic, copy) YKSelectBlock callBlock;
@property (nonatomic, copy) YKSelectBlock messageBlock;
...
- (id)initWithFrame:(CGRect)frame nodes:(NSArray*)nodes rootNodeID:(NSString*)rootID needPreservation:(BOOL)need selectBlock:(YKSelectBlock)block callBlock:(YKSelectBlock)callBlock messageBlock:(YKSelectBlock)messageBlock{
...
- (UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (cell.node.leaf) {
        ...
        cell.leftImage.frame = CGRectMake(0, 0, 0, 0);
       
    }else{
        
        cell.leftImage.hidden = NO;
    }
    if (cell.node.level==3) {
    ...
    cell.callButton.frame = CGRectMake(self.bounds.size.width - 35-rightMargin, 8, 35, 35);
    cell.messageButton.frame = CGRectMake(self.bounds.size.width - cell.callButton.frame.size.width - rightMargin - 35 - rightMargin, 8, 35, 35);
    [cell.messageButton addTarget:self action:@selector(messageClick:) forControlEvents:UIControlEventTouchUpInside];
    [cell.callButton addTarget:self action:@selector(callClick:) forControlEvents:UIControlEventTouchUpInside];
    return cell;
}
...
-(void)callClick:(UIButton *) button {
    YKNodeModel *currentNode = [_tempNodes objectAtIndex:button.tag];
    if (currentNode.isLeaf) {
        self.callBlock(currentNode);
    }

...
class QianTuiViewController: UIViewController,UICollectionViewDataSource,UICollectionViewDelegate,UICollectionViewDelegateFlowLayout{
    ...
    func setFooterView() -> Void {
        ...
        selectAllbtn.setTitle("签退", forState: UIControlState.Normal)
            ...
            if(error != nil){
                ...
                if(status.status == "success"){
                    ...
                    for item in self.dataSource.objectlist{
                        if item.checkedTypeT == "0"{
                            dao = dao + 1
                        }
                        if item.checkedTypeT == "1" || item.checkedTypeT == "2"{
                            weidao = weidao + 1
                        }
                    }
                    
                    self.sumCount.text = "总到:\(self.dataSource.objectlist.count)"
    ...
    func collectionView(collectionView: UICollectionView, cellForItemAtIndexPath indexPath: NSIndexPath) -> UICollectionViewCell {
        ...
        if (self.dataSource.objectlist[indexPath.row].checkedTypeT=="0"){
            cell.flag.hidden = false
            cell.flag.image = UIImage(named: "")
        }else if(self.dataSource.objectlist[indexPath.row].checkedTypeT=="1"){
            cell.flag.hidden = false
            cell.flag.image = UIImage(named: "ic_wei")
        }else if (self.dataSource.objectlist[indexPath.row].checkedTypeT=="2"){
            cell.flag.hidden = false
    ...
    func collectionView(collectionView: UICollectionView, didSelectItemAtIndexPath indexPath: NSIndexPath) {
        if(self.dataSource.objectlist[indexPath.row].checkedTypeT=="1"){
            self.dataSource.objectlist[indexPath.row].checkedTypeT="2"
        }else if (self.dataSource.objectlist[indexPath.row].checkedTypeT=="2"){
            self.dataSource.objectlist[indexPath.row].checkedTypeT="1"
        }
    ...
    func footerSelectAllBtnClick() -> Void {
        ...
        let url = "http://wxt.xiaocool.net/index.php?g=apps&m=index&a=LeaveTime"
        let param = [
            "userid":ids!,
            "schoolid":schoolid!,
            "sign_date":stringToTimeStamp(sign_date)
        ]
        Alamofire.request(.GET, url, parameters: param).response { request, response, json, error in
            if(error != nil){
            }

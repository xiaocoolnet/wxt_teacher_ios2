...
@interface ChetNewsViewController ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate>
...
@end
@implementation ChetNewsViewController
...
-(void)getData{
    ...
    NSString *chid = [[NSUserDefaults standardUserDefaults] stringForKey:@"userid"];
    NSString *studentid = [[NSUserDefaults standardUserDefaults] stringForKey:@"chid"];
    //服务器给的域名
    NSString *domainStr = @"http://wxt.xiaocool.net/index.php?g=apps&m=message&a=xcGetChatData";
    ...
    //往字典里面添加需要提交的参数
    [parametersDic setObject:chid forKey:@"send_uid"];
    [parametersDic setObject:self.receive_uid forKey:@"receive_uid"];
    ...
    [session GET:domainStr parameters:parametersDic success:^(NSURLSessionDataTask *task, id responseObject) {
        ...
        if ([model.status isEqualToString:@"success"]) {
            ...
            for (int i=0; i<model.data.count; i++) {
                ...
                dicValues[@"desc"]=[NSString stringWithFormat:@"%@",[model.data[i] objectForKey:@"content"]];
                dicValues[@"time"]=[model.data[i] objectForKey:@"time"];
                
                if ([[NSString stringWithFormat:@"%@",[model.data[i] objectForKey:@"send_uid"]] isEqualToString:chid]){
                ...
                NSLog(@"%@",[NSString stringWithFormat:@"%@",[model.data[i] objectForKey:@"time"]]);
                if ([[NSString stringWithFormat:@"%@",[model.data[i] objectForKey:@"time"]] isEqualToString:@"(null)"]) {
                    NSLog(@"%@",dicValues);
...
-(void)gameOver:(NSNotification *)text{
    
    NSString *title = [NSString stringWithFormat:@"%@",[text.object valueForKey:@"type"]];
    if ([title isEqualToString:@"newMessage"]) {
...
- (void)viewDidLoad {
    ...
    //注册通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(gameOver:) name:@"tongzhi" object:nil];
...
- (void)sendAction:(UIButton *)sender {
    ...
    NSDictionary *parameters = @{@"send_uid":userid,@"receive_uid":_receive_uid,@"content":_inputMess.text,@"send_type":@"1",@"usertype":self.usertype};
